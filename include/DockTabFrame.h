		...
		bool toggleClientViewState( HWND clientViewWnd, ClientView::State state) {
			...
			if (    dockUNKNOWN   == clientView->dockSide
				...
				return false;
			     ...
			     && (    ( stateFloating == state && stateFloating != clientView->state)
			          || ( stateDockable == state && stateFloating == clientView->state)
			        )
			   ) {
				...
				dropParcel = NULL;
			            ...
			            && stateDockable == state
			            && stateDockable != clientView->state
			          ) {
			...
			} else if (    stateFloating == state
			            && stateFloating != clientView->state
			...
			switch ( state) {
			case stateDockable:
				clientView->state = ( stateDockable == clientView->state)? stateNull: stateDockable;
				break;
			case stateFloating:
				clientView->state = ( stateFloating == clientView->state)? stateNull: stateFloating;
				break;
			}
				...
				case placeDOCKPANE:
					...
					if ( NULL != ( position = this->clientViews.GetStartPosition()))
						do {
							...
							if ( clientView->owner == splitPane)
								clientView->dockSide = dropParcel->sourceDock;
							if ( stateFloating == clientView->state) {
						...
						do {
							...
							if ( clientView->owner == splitPane) {
								...
								if ( stateFloating == clientView->state)
									...
									splitPane->detachClientView( clientView->wnd);
									if ( dockUNKNOWN != clientView->dockSide && stateDockable == clientView->state) {
